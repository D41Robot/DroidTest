...
short portIndex = 2; /* determines the USB port used */
int[] val = {-1, -1, -1, -1, -1}; /* Variable used for getY function, 5 positions for 5channels */
...
int[] getData()
    {
      int[]ChValues = {-1, -1, -1, -1, -1}; /* Define array for this function */
    ...
    for (int i=1; i<displayWidth-1; i++) /* Loop that runs from point k to the very right side of window */
      {
        ...
        int ya1 = constrain(getY(valuesCh1[k])+((height/5)*0), ((height/5)*0), ((height/5)*1)); /* Get next y-value for channel1 */
        int yb1 = constrain(getY(valuesCh2[k])+((height/5)*1), ((height/5)*1), ((height/5)*2)); /* Get next y-value for channel2 */
        int yc1 = constrain(getY(valuesCh3[k])+((height/5)*2), ((height/5)*2), ((height/5)*3)); /* Get next y-value for channel3 */
        int yd1 = constrain(getY(valuesCh4[k])+((height/5)*3), ((height/5)*3), ((height/5)*4));
        int ye1 = constrain(getY(valuesCh5[k])+((height/5)*4), ((height/5)*4), ((height/5)*5));
        ...
        stroke(0, 255, 255); /* Draw a cyan line for channel4 */
        line(x0, yd0, x1, yd1); /* Plot a line segment for channel4 */
        stroke(255, 0, 255); /* Draw a purple line for channel5 */
        line(x0, ye0, x1, ye1); /* Plot a line segment for channel5 */
